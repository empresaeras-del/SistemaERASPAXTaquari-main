...
if "useColumnVisibility" not in content:
    content = content.replace("import { Receipt, Search, Filter, Plus, Calendar, ArrowUpRight, DollarSign, Clock, CheckCircle2, ChevronRight, XCircle, FileDown, MoreHorizontal, ArrowRight, Printer } from 'lucide-react';", "import { Receipt, Search, Filter, Plus, Calendar, ArrowUpRight, DollarSign, Clock, CheckCircle2, ChevronRight, XCircle, FileDown, MoreHorizontal, ArrowRight, Printer } from 'lucide-react';\nimport { useColumnVisibility } from \"../hooks/useColumnVisibility\";\nimport { ColumnVisibilityToggle } from \"../components/ColumnVisibilityToggle\";")
...
if "<ColumnVisibilityToggle" not in content:
    nova_button = """<button 
            onClick={() => navigate('/financeiro/contas-receber/nova')}
...
content = content.replace("""<th 
                ...
                >
                  <div className="flex items-center gap-2">
                    Devedor / Origem
                    {sortField === 'devedor' && (
                      <span className="text-[#3B82F6]">{sortDirection === 'asc' ? '↑' : '↓'}</span>
                    )}
                  </div>
                </th>""", """{isVisible('devedor') && <th 
                  className="px-6 py-4 cursor-pointer hover:bg-bg-hover transition-colors"
                  onClick={() => {
                    if (sortField === 'devedor') {
                      setSortDirection(sortDirection === 'asc' ? 'desc' : 'asc');
...
content = content.replace("""<td className="px-6 py-4">
                    ...
                    </td>""", """{isVisible('devedor') && <td className="px-6 py-4">
                      <div className="font-medium text-text-base">
                        {p.devedor_nome}
                      </div>
                      <div className="text-xs text-text-subtle mt-0.5 capitalize">
...
content = content.replace("""<td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        {p.status === 'recebido' && p.data_pagamento && (
...
content = content.replace("""<td className="px-6 py-4 text-right">
                      ...
                      {p.status === 'recebido' && p.valor_recebido && p.valor_recebido !== p.valor && (
                        <div className="text-xs text-emerald-400 mt-0.5">
                          Rec. R$ {p.valor_recebido.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </div>
                      )}
...
content = content.replace("""<td className="px-6 py-4 text-center">
                      <button
                        onClick={() => navigate(`/financeiro/contas-receber/${p.id}`)}
