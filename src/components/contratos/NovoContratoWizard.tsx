...
export const NovoContratoWizard: React.FC<{
  ...
  // Formulário
  const [selectedAssociado, setSelectedAssociado] = useState<Associado | undefined>(
    associadoInicial,
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedAssociado.plano_pax_id && selectedAssociado.plano_pax_id !== planoId) {
        const hist = selectedAssociado.historico_contratos
          ? [...selectedAssociado.historico_contratos]
          : [];
        hist.push({
          id: uuidv4(),
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-text-subtle">
                Gere a identificação e configure as mensalidades
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex items-center px-6 py-4 border-b border-border-default bg-bg-subtle/50">
          {[
            ...
            <React.Fragment key={s.num}>
              <div
                className={`flex items-center gap-2 ${step === s.num ? 'text-[#3B82F6]' : step > s.num ? 'text-emerald-500' : 'text-text-muted'}`}
              >
                <div
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {a.plano_pax_id && (
                            <span className="inline-flex mt-2 text-[10px] bg-amber-500/10 text-amber-500 px-2 py-0.5 rounded font-bold">
                              Já possui plano
                            </span>
                          )}
                        </div>
                ...
                <div className="bg-bg-subtle rounded-xl p-4 space-y-2 border border-border-default">
                  ...
                  <div className="flex justify-between items-center text-sm border-b border-border-default/50 pb-2">
                    ...
                    <span className="font-semibold text-text-base">
                      {1 + (selectedAssociado?.dependentes?.length || 0)} Vidas
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm border-b border-border-default/50 pb-2 pt-2">
                    ...
                    <span className="font-semibold text-text-base">
                      R$ {(planoSelecionado.taxa_adesao || 0).toFixed(2).replace('.', ',')}
                    </span>
                  </div>
          ...
          {step === 4 && (
            <div className="space-y-6">
              ...
              <p className="text-sm text-text-subtle">
                Solicite a assinatura do associado para o contrato.
              </p>

              <div className="bg-bg-subtle border border-border-default rounded-xl p-4 flex flex-col items-center">
                <div
                  className="w-full max-w-[600px] border-2 border-dashed border-border-default bg-white rounded-xl overflow-hidden touch-none"
                  style={{ height: '300px' }}
                >
                  <canvas
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <p className="text-sm text-text-subtle">
                Programe as cobranças iniciais para o contrato gerado.
              </p>
              ...
              {ultrapassouLimiteColetivo && (
                <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl flex flex-col gap-2">
                  <div className="flex items-center gap-2 text-amber-500 font-medium">
                    ...
                    {/* just to import safely, use an alert icon or text */}
                    <span>Atenção: Limite de Vidas Excedido</span>
                  </div>
                  <p className="text-sm text-text-subtle">
                    A quantidade de vidas cadastradas é superior ao máximo permitido (
                    {planoSelecionado?.limite_vidas}) para este plano coletivo. Adicione um valor
              ...
              <div className="bg-bg-subtle border border-border-default rounded-xl p-4 overflow-hidden">
                ...
                <div className="max-h-[250px] overflow-y-auto space-y-2 pr-2">
                  {parcelas.map((p, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3 bg-bg-surface border border-border-default rounded-lg"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-text-subtle">
                            Vence em: {p.data_vencimento.split('-').reverse().join('/')}
                          </p>
                        </div>
                      </div>
              ...
              onClick={() => {
                if (step === 1 && selectedAssociado?.plano_pax_id) {
                  confirm({
                    ...
                    message:
                      'O associado selecionado já possui um contrato com plano ativo. Por favor, selecione outro associado para dar seguimento.',
                    confirmText: 'Entendi',
                    onConfirm: () => {},
                  });
                  return;
