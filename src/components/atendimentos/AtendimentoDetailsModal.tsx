...
import {
  cancelarReceitasPorAtendimento,
  getParcelasReceberPorAtendimento,
  ParcelaReceber,
...
import {
  ...
  isAtendimentoExcluivel,
  alertPermissionRestriction,
...
export const AtendimentoDetailsModal: React.FC<Props> = ({ atendimento, onClose, onSaved }) => {
  ...
  const executeSave = async () => {
    ...
    try {
      ...
      const newValorTotal = itemsList.reduce(
        (acc, curr) => (curr.coberto ? acc : acc + curr.quantidade * curr.valor_unitario),
        0,
      );
  ...
  const handleSave = async () => {
    if (formData.status === 'cancelado' && atendimento.status !== 'cancelado') {
      confirm({
        ...
        message:
          'Ao cancelar este atendimento, todas as parcelas financeiras pendentes (contas a receber) vinculadas a ele serão automaticamente canceladas. Deseja prosseguir?',
        ...
        <div className="px-6 py-4 border-b border-border-default flex justify-between items-center bg-bg-surface shrink-0">
          <div>
            ...
            <p className="text-sm text-text-subtle">
              {formData.tipo_cliente === 'associado' ? 'Associado' : 'Cliente Externo'} •{' '}
              {new Date(formData.created_at || '').toLocaleDateString()}
            </p>
          </div>
          <div className="flex items-center gap-3">
            {!isEditing &&
              canDeleteAtendimento(state.user, state.isOnline) &&
              isAtendimentoExcluivel(formData.status) && (
                <button
                  type="button"
              ...
              <button
                ...
                onClick={() => {
                  if (!canEditAtendimentos(state.user, state.isOnline)) {
                    alertPermissionRestriction(
                      'Atendimentos',
                      'editar ou alterar dados de atendimentos cadastrados',
                    );
                    return;
                  }
                  setIsEditing(true);
          ...
          {activeTab === 'detalhes' ? (
            <>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <input
                            ...
                            value={formData.falecido_data_nascimento || ''}
                            onChange={handleChange}
                            className="w-full px-3 py-2 bg-bg-subtle border border-border-default rounded-lg text-sm"
                          />
                        ) : (
                          <p className="text-sm text-text-base font-medium">
                            {formatLocalDate(formData.falecido_data_nascimento)}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <p className="text-sm text-text-base font-medium">
                            {formData.data_obito
                              ? new Date(formData.data_obito).toLocaleString()
                              : '-'}
                          </p>
                        )}
                      </div>
                      <div>
                        ...
                        {isEditing ? (
                          <input
                            type="datetime-local"
                            name="data_velorio"
                            value={formData.data_velorio || ''}
                            onChange={handleChange}
                            className="w-full px-3 py-2 bg-bg-subtle border border-border-default rounded-lg text-sm"
                          />
                        ) : (
                          <p className="text-sm text-text-base font-medium">
                            {formData.data_velorio
                              ? new Date(formData.data_velorio).toLocaleString()
                              : '-'}
                          </p>
                        )}
                      </div>
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <p className="text-sm text-text-base font-medium">
                            {formData.data_sepultamento
                              ? new Date(formData.data_sepultamento).toLocaleString()
                              : '-'}
                          </p>
                        )}
                      </div>
              ...
              <div className="bg-bg-surface rounded-xl border border-border-default p-5 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      <input
                        name="declaracao_obito"
                        value={formData.declaracao_obito || ''}
                        onChange={handleChange}
                        className="w-full px-3 py-2 bg-bg-subtle border border-border-default rounded-lg text-sm focus:ring-2 focus:ring-primary/50"
                      />
                    ) : (
                      <p className="text-sm text-text-base font-medium">
                        {formData.declaracao_obito || '-'}
                      </p>
                    )}
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      <input
                        type="time"
                        name="inicio_tanato"
                        value={formData.inicio_tanato || ''}
                        onChange={handleChange}
                        className="w-full px-3 py-2 bg-bg-subtle border border-border-default rounded-lg text-sm"
                      />
                    ) : (
                      <p className="text-sm text-text-base font-medium">
                        {formData.inicio_tanato || '-'}
                      </p>
                    )}
                  </div>
                  <div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-border-default/50">
                      {itemsList.map((item, idx) => {
                        ...
                        const nomeItem = itemDef
                          ? itemDef.nome
                          : item.item_nome || 'Item Desconhecido';
                            ...
                            <td className="py-3 text-text-base text-center">
                              {isEditing ? (
                                <input
                                  ...
                                  onChange={(e) =>
                                    handleItemChange(idx, 'quantidade', Number(e.target.value))
                                  }
                            ...
                            <td className="py-3 text-center">
                              {isEditing ? (
                                <select
                                  ...
                                  onChange={(e) =>
                                    handleItemChange(idx, 'coberto', e.target.value === 'true')
                                  }
                            ...
                            {isEditing && (
                              <td className="py-3 text-center">
                                <button
                                  ...
                                  type="button"
                                  className="p-1.5 text-text-subtle hover:text-rose-500 hover:bg-rose-500/10 rounded transition-colors"
                                  title="Remover Item"
                                >
                      ...
                      {itemsList.length === 0 && (
                        <tr>
                          <td
                            ...
                            className="py-6 text-center text-text-subtle"
                          >
                            Nenhum item adicionado a este atendimento.
                          </td>
                        </tr>
                      )}
                    </tbody>
              ...
              <div className="bg-bg-surface rounded-xl border border-border-default overflow-hidden shadow-sm mt-6">
                ...
                <div className="p-4 overflow-x-auto">
                  {loadingParcelas ? (
                    <div className="text-center text-text-subtle text-sm py-4">
                      Carregando parcelas...
                    </div>
                  ) : parcelas.length === 0 ? (
                    <div className="text-center text-text-subtle text-sm py-4">
                      Nenhuma parcela vinculada a este atendimento.
                    </div>
                  ) : (
                    <table className="w-full text-sm">
                      <thead>
                      ...
                      <tbody>
                        {parcelas.map((p, idx) => (
                          <tr
                            key={p.id}
                            className="border-b border-border-default/50 last:border-0 hover:bg-bg-subtle/50 transition-colors"
                          >
                            ...
                            ${
                              p.status === 'pago' || p.status === 'recebido'
            ...
            <div className="flex items-center gap-2 text-xs text-text-subtle">
              {isEditing ? (
                <span className="flex items-center gap-1.5 text-amber-500 font-medium bg-amber-500/10 px-2.5 py-1 rounded-lg border border-amber-500/20">
                  <Edit2 className="w-3.5 h-3.5" /> Modo de edição ativo: faça as alterações e
                  clique em Salvar.
                </span>
              ) : (
